...
export default function ComparePage() {
  ...
  const fetchData = async () => {
    ...
    const { data: d1, error: e1 } = await fetchHistoricalData(country1, indicator);
    const { data: d2, error: e2 } = await fetchHistoricalData(country2, indicator);
    ...
    if (d1.length > 0 && d2.length > 0) {
      const allDates = [...d1, ...d2].map((d) => new Date(d.DateTime));
  ...
  const handleFilter = () => {
    ...
    const filtered1 = originalData1.filter(
      (d) => new Date(d.DateTime) >= sDate && new Date(d.DateTime) <= eDate
    );
    const filtered2 = originalData2.filter(
      (d) => new Date(d.DateTime) >= sDate && new Date(d.DateTime) <= eDate
    );
  ...
  const chartData = {
    labels: data1?.map((d) => formatDate(d.DateTime)),
    datasets: [
      {
        label: country1,
      ...
      },
      {
        ...
        borderColor: '#ef4444',
        backgroundColor: (ctx) => getGradient(ctx.chart.ctx, '#ef4444'),
        fill: true,
        tension: 0.4,
        pointRadius: 2,
      },
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 md:p-12">
      ...
      <h1 className="text-4xl font-semibold text-center text-gray-800 mb-4">
        📊 Country Indicator Comparison

      </h1>
      <p className="text-center text-gray-500 mb-8 italic">
        Free accounts support: Mexico, New Zealand, Sweden, Thailand
        <br />
        <span className='mt-4'>Working Ex : Country 1: Sweden, Country2: New Zealand, Indicator: Bankruptcies </span>
      </p>
        ...
        <div className="min-w-[1000px] relative" style={{ height: '500px' }}>
          {data1.length > 0 && data2.length > 0 ? (
            <div className="h-[400px]">
              <h2 className="text-xl font-semibold mb-4 text-center">{data1[0].Category}</h2>
              <Line data={chartData} options={chartOptions} />
            </div>
          ) : (
            <p className="text-center text-gray-400 text-lg">
              Please select options and click "Compare" to see results.
            </p>
          )}
        </div>
      </div>
